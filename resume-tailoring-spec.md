...
- `resume.filename_pattern` — e.g. `"Raj Khettry SWE {company}"`. Replace `{company}` with the target company name when writing the output `.md` / `.tex` / `.pdf` files.
...
Throughout this spec, when a section mentions a specific value (e.g. "Raj Khettry SWE", "$134k/yr GPU saving", "PlateMax"), **use the values from user.yaml**, not the inline ones. The inline values are illustrative defaults for the template owner.
...
**Output:** `<REPO_ROOT>/versions/<slug>/` containing files matching `resume.filename_pattern` from config (e.g. `Raj Khettry SWE <Company>.md`, `.tex`, `.pdf`)

---

## 1. Hard quality rules (non-negotiable)

These are what Raj means by "no white space" and "good 1 page resume." Verification step in §5 enforces them.
...
4. **Every bullet follows the Google XYZ formula**: `Accomplished [X] as measured by [Y] by doing [Z]`. In practice this means each bullet must have (a) the action verb / what was built, (b) the metric / impact (`$134k/yr saved`, `80% latency drop`, `10k+ emails/day`, `300+ interviews`, `placed 6 hires`), and (c) the technical specifics (named technologies, architecture details). The master resume's existing bullets already follow this pattern — match the density. Bullets missing any of (X, Y, Z) = rewrite using the source bullet from `experiences/`.
5. **Preserve the killer metrics from the master.** These numbers are the user's strongest signals and must stay visible across every tailored version unless the role genuinely doesn't care: **$134k/yr GPU cost saving (Dell)**, **$75k/yr LLM cost saving (WPM)**, **80% latency reduction 40min→8min (WPM)**, **300+ AI interviews / 6 hires placed (Loop)**, **10k+ emails/day & 1,500+ users (WPM)**, **8-GPU Kubernetes cluster (Dell)**. Tailoring = re-emphasizing, never silently dropping these.
6. **Tailored content only from `experiences/`, `skills.md`, `profile.md`, and the master resume.** Do NOT fabricate metrics, projects, or accomplishments. If a job calls for something Raj doesn't have, leave it out — never invent.
...
**Reordering rule:** Capital One (current) stays first. Dell stays second by default (offer-in-hand signal). But for some companies, lead differently:
- **Voice AI / agentic / Sierra-like companies** → promote Loop above Dell (voice pipeline is the killer story).
- **Multimodal / vision / Krea-like** → keep Dell first (multimodal RAG is the headline).
- **Dev tools / infra / Modal-like** → keep Dell first; reframe Loop bullets around infra.
- **Healthcare AI** → keep Dell first (Dell healthcare RAG); reframe WPM if useful.
- **Frontier labs (Anthropic, OpenAI, etc.)** → keep Capital One first (current shipping role), Dell second.

**Bullet selection per role (pick the strongest 2–3 for the target):**
- **Capital One:** always include the RAG bullet. Optionally add the PagerDuty/observability bullet.
- **Dell:** at least the multimodal RAG bullet. Add the $134k GPU optimization bullet for infra/cost-conscious shops. Add the K8s data pipeline bullet for platform roles.
- **Loop:** voice pipeline bullet for voice/agentic roles. Conversational agent bullet for AI-product roles. The "owned full stack" bullet only if Loop is featured AND there's space; otherwise drop.
- **WPM:** knowledge-graph RAG bullet for RAG/search roles. Email insights bullet for scale/infra roles. If Loop is featured, often just one WPM bullet.

**Skills section tailoring:**
- 4 categories shown (AI/ML, ML Infrastructure, Languages, Backend/Cloud). Keep all 4.
- Reorder if relevant: for ML platform roles, move ML Infrastructure ABOVE AI/ML.
- Drop/add specific items from `skills.md` based on JD keywords. Don't add anything Raj hasn't actually done.

**Projects section:**
- PlateMax stays. It's universally strong (real users, real outcomes, App Store). Description line may be re-framed per role (consumer iOS vs ML-powered vs multimodal inference) — same project, different lens.

**Organizations section:**
- Keep both (AEPi, UCSC Startup Club). They take 2 lines and prove leadership.
...
| Archetype | JD signals | Lead verbs to use | Skills emphasis | PlateMax framing |
|---|---|---|---|---|
...
- **Reframe the same work with a different domain lens.** The Dell K8s pipeline IS, simultaneously and truthfully: "distributed systems work" / "ML infrastructure" / "platform engineering" / "data pipeline at scale" / "customer deployment infra". Pick the lens that matches the archetype.
- **Re-frame project description lines per archetype.** PlateMax has multiple honest framings (see the table above) — pick the archetype-matching one.
...
- ❌ **No new tools/technologies.** If Raj didn't actually use Kafka, don't mention Kafka — even if the JD demands it. Leave it for the cover letter as a learning interest.
- ❌ **Killer metrics stay visible** ($134k Dell, $75k WPM, 80% latency WPM, 300+/6 hires Loop, 10k+ emails WPM, 8-GPU cluster Dell). Re-frame them — never silently drop them.
...
- ❌ **Don't force an archetype that doesn't fit the facts.** If the JD is FDE-flavored but the candidate has no real customer-deployment story → don't invent one. Use the archetype's lead verbs only where they're truthful (e.g. "Deployed" is fine for the Dell one-click customer deployment bullet but NOT for the WPM internal email pipeline). When the archetype reframe would distort a fact, fall back to honest framing — even if it costs you JD-keyword coverage.
...
├── Raj Khettry SWE Upstart.md     # markdown source + tailoring rationale at bottom
├── Raj Khettry SWE Upstart.tex    # LaTeX (copy of master.tex with bullets/skills swapped)
├── Raj Khettry SWE Upstart.pdf    # compiled output — THIS is what Raj sends
...
**Filename convention** (THE deliverable filename, used for .md/.tex/.pdf so they all match if Raj forwards the source). **Never name the deliverable files `tailored.*` — always use this pattern:**

```
Raj Khettry SWE <Company>.<ext>
```
...
- Company `OpenAI` → `Raj Khettry SWE OpenAI.pdf`
- Company `Anthropic` → `Raj Khettry SWE Anthropic.pdf`
- Company `Meta` → `Raj Khettry SWE Meta.pdf`

**Folder slug rule** (the containing directory — Raj never sees this, it's just for filesystem tidiness): `<company-lowercase-hyphenated>-<role-lowercase-hyphenated-trimmed-to-40-chars>`. Strip punctuation. Examples:
...
**`Raj Khettry SWE <Company>.md` body structure:**
```markdown
# Raj Khettry — Tailored for <Company> — <Role>
...
iteration = 1
while iteration <= 4:
    1. Compile: cd <REPO_ROOT>/versions/<slug>/ && pdflatex -interaction=nonstopmode "Raj Khettry SWE <Company>.tex"
       (If pdflatex is unavailable, `tectonic "Raj Khettry SWE <Company>.tex"` is an accepted drop-in engine.)
       - On compile error, fix the .tex and retry. Compile errors don't count as an iteration.
    2. Read the resulting "Raj Khettry SWE <Company>.pdf" (use the Read tool with the PDF path — Claude can view PDFs visually).
...
```
open "<REPO_ROOT>/versions/<slug>/Raj Khettry SWE <Company>.pdf"
```
This opens the PDF in macOS Preview so Raj can eyeball it. Skip on headless / errored runs.
...
pdf_path = "<REPO_ROOT>/versions/<slug>/Raj Khettry SWE <Company>.pdf"
...
## 9. Dials Raj may want to tweak over time
...
- The default "Capital One first" lead
